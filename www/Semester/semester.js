  ...
  for (let index = 0; index < snapshot.docs.length; index += 450) {
    ...
    snapshot.docs.slice(index, index + 450).forEach(doc => {
      ...
      if (kelasParts.tingkat === "9") {
        const lulusRef = documentsApi.collection("siswa_lulus").doc(makeSiswaLulusDocId(tahunLulus, nipd));
        batch.set(lulusRef, {
          ...data,
          nipd,
          kelas_lulus: kelasParts.kelas || data.kelas || "",
      ...
      batch.set(getSemesterDocRef("siswa", nipd, active.id), {
...
async function rebuildPreviousSiswaSnapshot(previous, active) {
  let count = 0;
  const documentsApi = getSemesterDocumentsApi();
  const siswaSnapshot = await getSemesterSourceSnapshot(active, "siswa");
  for (let index = 0; index < siswaSnapshot.docs.length; index += 450) {
    ...
    siswaSnapshot.docs.slice(index, index + 450).forEach(doc => {
      ...
      const nipd = data.nipd || doc.id;
      if (!nipd) return;
      const previousData = cloneStudentForPreviousSnapshot({ id: nipd, ...data, nipd });
...
async function snapshotSemesterData(term) {
  if (!term?.id) return;
  const active = getSemesterSettingsList().find(item => item.id === semesterAdminState.active_id) || getDefaultSemesterContext();
  await snapshotSemesterCollection(term.id, "siswa", active);
  await snapshotSemesterCollection(term.id, "kelas", active);
}

async function snapshotSemesterCollection(termId, collectionName, sourceTerm = null) {
  const documentsApi = getSemesterDocumentsApi();
  const snapshot = await getSemesterSourceSnapshot(sourceTerm || getActiveSemesterContext(), collectionName);
  let count = 0;
  for (let index = 0; index < snapshot.docs.length; index += 450) {
    const batch = documentsApi.batch();
    snapshot.docs.slice(index, index + 450).forEach(doc => {
      const data = doc.data();
      const docId = data?.nipd || data?.kelas || doc.id;
      if (!docId) return;
      count++;
      batch.set(
        documentsApi.collection("semester_data").doc(termId).collection(collectionName).doc(docId),
        {
          ...data,
          id_asli: docId,
          term_id: termId,
...
async function clearAllWaliKelas() {
  ...
  for (let index = 0; index < snapshot.docs.length; index += 450) {
    ...
    snapshot.docs.slice(index, index + 450).forEach(doc => {
      const data = doc.data();
      const kelasId = data.kelas || doc.id;
      if (!kelasId) return;
      batch.set(getSemesterDocRef("kelas", kelasId, active.id), {
        kode_guru: "",
        wali_kelas: "",
        updated_at: new Date()
      }, { merge: true });
