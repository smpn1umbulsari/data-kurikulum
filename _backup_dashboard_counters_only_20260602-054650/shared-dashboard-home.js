(function initDashboardHome(global) {
  ...
  function getHomeCacheKey(context = {}) {
    return [
      ...
      Array.isArray(context.coordinatorLevels)
        ? context.coordinatorLevels.join("-")
        : "",
    ].join(":");
  }
  ...
  function readHomeSnapshot(context = {}) {
    try {
      return JSON.parse(
        global.localStorage.getItem(getHomeCacheKey(context)) || "null",
      );
    } catch {
      return null;
    }
  ...
  function captureHomeSnapshot(context = {}) {
    try {
      const nodes = [
        ...global.document.querySelectorAll("[id^='home'], [id^='guruHome']"),
      ];
      const values = nodes
        .filter((node) => node.id)
        .map((node) => ({
  ...
  function setHomeRefreshState(isRefreshing = false) {
    ...
    global.document
      ?.querySelectorAll?.(".home-refresh-btn")
      .forEach((button) => {
    ...
    return withDashboardFetchCache(`collection:${collectionName}`, async () => {
      ...
      const query =
        typeof queryFactory === "function"
          ? queryFactory()
          : documentsApi.collection(collectionName);
  ...
  async function getCachedDocumentRow(collectionName, docId) {
    return withDashboardFetchCache(
      `document:${collectionName}:${docId}`,
      async () => {
        const snapshot = await getDocumentsApi()
  ...
  function escapeHtml(value) {
    return global.AppUtils?.escapeHtml
      ? global.AppUtils.escapeHtml(value)
      : String(value ?? "");
  }

  function getKelasParts(kelasValue = "") {
    return global.AppUtils?.parseKelas
      ? global.AppUtils.parseKelas(kelasValue)
      : { tingkat: "", rombel: "", kelas: String(kelasValue || "") };
  }

  function getSiswaKelasBayanganParts(siswa) {
    return global.AppUtils?.getPrimaryKelasParts
      ? global.AppUtils.getPrimaryKelasParts(siswa)
      : getKelasParts(siswa.kelas);
  }

  function getJenisKelamin(siswa) {
    const raw = String(
      siswa.jenis_kelamin || siswa.jk || siswa.gender || siswa.kelamin || "",
    )
      .trim()
      .toLowerCase();
    if (["l", "lk", "laki", "laki-laki", "laki laki", "1"].includes(raw))
      return "L";
  ...
  function formatGuruName(guru, fallbackUser = {}) {
    if (guru && typeof global.formatNamaGuru === "function")
      return global.formatNamaGuru(guru);
    return (
      guru?.nama ||
      fallbackUser.nama ||
      fallbackUser.guru_nama ||
  ...
  async function loadHomeInvigilationCard(documentsApi, kodeGuru = "") {
    ...
    const snapshot = await documentsApi
      .collection("kepangawasan_kartu_guru")
      .doc(docId)
      .get();
  ...
  function isPresenceOnline(record = {}, thresholdMs = 180000) {
    ...
    return (
      ...
      Date.now() - seenAt <= Number(thresholdMs || 180000)
    );
  }
  ...
  async function loadHomeNilaiRows(documentsApi, options = {}) {
    if (
      !options.force &&
      typeof documentsApi?.getPersistedCollectionRowsAsync === "function"
    ) {
      const cachedRows =
  ...
  async function loadHomeStats(options = {}) {
    ...
    try {
      ...
      const siswa = siswaSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const kelas = kelasSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const mapelAsli = mapelAsliSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const mengajarAsli = mengajarAsliSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const mapel = mapelSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const mengajar = mengajarSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const tugas = tugasSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      ...
      const roleAssignmentsAsli = getHomeVisibleAssignments(options, {
        mengajarBayangan: mengajarAsli,
        kelas,
      });
      const teachingRows = buildTeachingComparisonRows(
        roleAssignmentsAsli,
        mapelAsli,
        getHomeVisibleAssignments(options, {
          mengajarBayangan: mengajar,
          kelas,
        }),
        mapel,
      );
      const waliCount = kelas.filter(
        (item) =>
          String(item.kode_guru || "").trim() ||
          String(item.wali_kelas || "").trim(),
      ).length;
      const kelasBayanganCount = siswa.filter((item) =>
        String(item.kelas_bayangan || "").trim(),
      ).length;
      ...
      setText(
        "homeMengajarCount",
        `${mengajar.length} pembagian mengajar tersimpan`,
      );
      setText("homeWaliCount", `${waliCount}/${kelas.length} wali kelas`);
      setText(
        "homeKelasBayanganInfo",
        `${kelasBayanganCount} siswa memiliki kelas real manual`,
      );
      setText("homeTugasTambahanCount", `${tugas.length} tugas`);
      setText(
        "homeTugasTambahanInfo",
        `${tugas.length} tugas tambahan terdaftar`,
      );
      setText("homeOnlineCount", `${onlineUsers.length} user online`);
      setText(
        "homeUpdatedAt",
        `Data dimuat ${global.AppUtils?.formatDateTimeId ? global.AppUtils.formatDateTimeId(new Date()) : new Date().toLocaleString("id-ID")}`,
      );
      if (
        options.role === "guru" ||
        options.role === "koordinator" ||
        options.role === "guruadmin" ||
        ...
        if (teachingTitle && teachingTable) {
          ...
          teachingTable.innerHTML = renderTeachingTable(
            teachingRows,
            "Belum ada tugas mengajar yang bisa ditampilkan.",
          );
        }
      }
      global.AppDom?.setHtml?.(
        "homeSiswaByLevel",
        `
  ...
  async function loadGuruHomeStats(options = {}) {
    ...
    try {
      ...
      const guruList = guruSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      const siswa = siswaSnap.docs.map((doc) => ({
      ...
      const tugasGuru = tugasGuruSnap.exists
        ? { id: tugasGuruSnap.id, ...tugasGuruSnap.data() }
        : {};
      const guru =
        guruList.find(
          (item) => String(item.kode_guru || "").trim() === kodeGuru,
        ) || {};
      const myAssignments = mengajarAsli.filter(
        (item) => String(item.guru_kode || "").trim() === kodeGuru,
      );
      const myAssignmentsBayangan = mengajar.filter(
        (item) => String(item.guru_kode || "").trim() === kodeGuru,
      );
      const tugasNames = getTugasNames(tugasGuru, tugas);
      const waliRows = kelas.filter(
        (item) => String(item.kode_guru || "").trim() === kodeGuru,
      );
      const teachingRows = buildTeachingComparisonRows(
        myAssignments,
        mapelAsli,
        myAssignmentsBayangan,
        mapel,
      );
      global.DashboardShell?.setWaliAccessState?.(
        Boolean(waliRows.length),
        waliRows
          .map(
            (item) =>
              getKelasParts(
      ...
      setText(
        "guruHomeMengajarCountLabel",
        `${teachingRows.length} mapel diajar`,
      );
      ...
      global.AppDom?.setHtml?.(
        "guruHomeMengajarTable",
        renderTeachingTable(
          teachingRows,
          "Belum ada tugas mengajar kelas real.",
        ),
      );
      global.AppDom?.setHtml?.(
        "guruHomeTugasList",
        tugasNames.length
          ? tugasNames
              .map(
                (name) =>
                  `<span><strong>${escapeHtml(name)}</strong><b>aktif</b></span>`,
              )
              .join("")
      ...
      if (waliRows.length && waliSummary && waliInfo) {
        const waliKelas = getKelasParts(
          waliRows[0].kelas ||
            `${waliRows[0].tingkat || ""}${waliRows[0].rombel || ""}`,
        ).kelas;
        const waliStudents = siswa
          .map((item) => ({
            ...item,
            kelasBayanganParts: getSiswaKelasBayanganParts(item),
          }))
          .filter((item) => item.kelasBayanganParts.kelas === waliKelas);
        const genderCounts = waliStudents.reduce(
          (result, item) => {
  ...
  function getHomeUserDisplayName(user = {}) {
    return (
      user.nama ||
      user.username ||
      user.guru_nama ||
  ...
  function renderHomeStatCard(icon, label, value, iconClass = "") {
    return `
      <div class="stat-card">
  ...
  function renderHomeGreetingHeader(context = {}) {
    const userName = getHomeUserDisplayName(context.user);
    const formattedDate = getFormattedDate();
    const greetingText = context.isGuru
      ? `Selamat bekerja, ${escapeHtml(userName)}!`
      : `Selamat datang, ${escapeHtml(userName)}!`;
    const subText = context.isGuru
      ? "Pantau kelas yang Anda ajar dan catatan yang masih perlu dilengkapi."
      : context.isAdmin
        ? "Pantau rangkuman input, user online, dan sebaran data yang sedang berjalan."
        : context.isKoordinator
          ? "Pantau input lintas jenjang yang bisa Anda jangkau."
          : "Pantau data sekolah dari satu layar.";

    return `
      <div class="home-header">
        <div class="home-header-left">
          <h2 class="home-greeting">${greetingText}</h2>
          <p class="home-greeting-sub">${escapeHtml(subText)}</p>
        </div>
        <div class="home-date-card">
          <div class="home-date-icon">&#128197;</div>
          <div class="home-date-info">
            <span class="home-date-label">Tanggal</span>
            <span class="home-date-value">${escapeHtml(formattedDate)}</span>
          </div>
        </div>
      </div>
    `;
  ...
  function renderHomeStatCards(context = {}, stats = {}) {
    const siswaCount = String(stats.siswaCount || stats.siswa || 0);
    const kelasCount = String(stats.kelasCount || stats.kelas || 0);
    const guruCount = String(stats.guruCount || stats.guru || 0);
    const tugasCount = String(stats.tugasCount || stats.tugas || 0);
    const onlineCount = String(stats.onlineCount || stats.online || 0);

    return `
      <div class="stat-cards-grid">
        ${renderHomeStatCard("&#128100;", "Jumlah Siswa", siswaCount, "stat-card-icon--students")}
        ${renderHomeStatCard("&#127979;", "Jumlah Kelas", kelasCount, "stat-card-icon--class")}
        ${renderHomeStatCard("&#129333;", "Guru & Staf", guruCount, "stat-card-icon--staff")}
        ${renderHomeStatCard("&#128197;", "Jadwal Hari Ini", "0", "stat-card-icon--schedule")}
        ${renderHomeStatCard("&#128221;", "Tugas Dinilai", tugasCount, "stat-card-icon--tasks")}
      </div>
    `;
  }
  ...
  function getHomeRoleContext(options = {}) {
    ...
    const role = String(options.role || user.role || "admin")
      .trim()
      .toLowerCase();
    const hasCoordinatorAccess =
      typeof options.hasCoordinatorAccess === "function"
    ...
    const isKoordinator =
      role === "koordinator" || (isGuru && hasCoordinatorAccess);
  ...
  function resolveHomeGuruFromUser(user = {}, guruList = []) {
    ...
    if (global.AdminUsersIdentity?.getGuruForAdminUser) {
      const matched = global.AdminUsersIdentity.getGuruForAdminUser(
        user,
        guruList,
        (value) => getHomeGuruByKode(guruList, value),
      );
      if (matched) return matched;
    }

    const userAliases = [user?.username, user?.id, user?.nama]
    ...
    return (
      guruList.find((guru) => {
        const guruAliases = [
          guru?.kode_guru,
          guru?.nip,
          global.AdminUsersIdentity?.makeGuruUsername
            ? global.AdminUsersIdentity.makeGuruUsername(guru)
            : "",
          global.AdminUsersIdentity?.getGuruName
            ? global.AdminUsersIdentity.getGuruName(guru)
            : guru?.nama,
          global.AdminUsersIdentity?.getGuruUsernameName
            ? global.AdminUsersIdentity.getGuruUsernameName(guru)
            : guru?.nama,
        ]
          .map((value) => normalizeHomeIdentity(value))
          .filter(Boolean);
  ...
  function getHomeResolvedContext(context = {}, guruList = []) {
    if (
      context.kodeGuru ||
      !Boolean(context.isAdmin || context.role === "superadmin")
    )
      return context;
  ...
  function getHomeInputScopeContext(context = {}) {
    const shouldUseGuruScope =
      Boolean(context.kodeGuru) &&
      (context.isAdmin ||
  ...
  function getHomeHeroTitle(context) {
    if (context.isGuruAdmin)
      return "Pantau data sekolah sekaligus workflow guru.";
    if (context.isAdmin)
      return "Pantau rangkuman input dan status data sekolah.";
    if (context.isKoordinator)
      return `Pantau input lintas jenjang ${context.coordinatorLevels.length ? context.coordinatorLevels.join(", ") : "-"}.`;
    if (context.isGuru)
      return "Pantau kelas yang Anda ajar dan catatan yang masih perlu dilengkapi.";
    return "Pantau data sekolah dari satu layar.";
  }

  function getHomeHeroDescription(context) {
    if (context.isGuruAdmin)
      return "Beranda menampilkan rangkuman input, data sekolah, dan pintasan guru dalam satu layar.";
    if (context.isAdmin)
      return "Beranda menampilkan rangkuman input, user online, dan sebaran data yang sedang berjalan.";
    if (context.isKoordinator)
      return "Beranda menampilkan input yang bisa Anda jangkau, kelas wali, dan akses cepat ke data jenjang.";
    if (context.isGuru)
      return "Beranda menampilkan kelas mapel yang Anda ajar, status input, dan ringkasan wali kelas bila ada.";
  ...
  function getHomeSummaryStatLabel(context, index) {
    if (context.isGuruAdmin)
      return ["Guru", "Kelas", "User Online"][index] || "";
    if (context.isAdmin) return ["Guru", "Kelas", "User Online"][index] || "";
    if (context.isKoordinator)
      return ["Jenjang", "Kelas Wali", "Siswa"][index] || "";
    if (context.isGuru)
      return ["Kelas Mapel", "Tugas Tambahan", "Kelas Wali"][index] || "";
  ...
  function getHomeRoleCards(context) {
    ...
    if (context.isGuruAdmin) {
      return [{ label: "User Online" }, { label: "Sebaran Data" }];
    }
    if (context.isKoordinator) {
      return [{ label: "Jangkauan Koordinator" }, { label: "Kelas Wali" }];
    }
    return [{ label: "Tugas Mengajar" }, { label: "Wali Kelas" }];
  }

  function getHomeAssignmentKey(assignment = {}) {
    return [
      String(assignment.tingkat || "").trim(),
      String(assignment.rombel || "")
        .trim()
        .toUpperCase(),
      String(assignment.mapel_kode || "")
        .trim()
        .toUpperCase(),
  ...
  function getHomeAssignmentClassKey(assignment = {}) {
    ...
    const rombel = String(assignment.rombel || "")
      .trim()
      .toUpperCase();
  ...
  function isHomeNilaiDocMatchingAssignment(item = {}, assignment = {}) {
    if (
      typeof global.isActiveTermDoc === "function" &&
      !global.isActiveTermDoc(item)
    )
      return false;
  ...
  function getHomeStudentsForAssignment(
    assignment = {},
    siswaList = [],
    mapelList = [],
  ) {
    ...
    return siswaList
      .map((siswa) => ({
        ...siswa,
        kelasBayanganParts: getSiswaKelasBayanganParts(siswa),
      }))
      .filter((siswa) => {
        if (
          siswa.kelasBayanganParts.tingkat !== String(assignment.tingkat || "")
        )
          return false;
        if (
          siswa.kelasBayanganParts.rombel !==
  ...
  function getHomeVisibleAssignments(context, data = {}) {
    const assignments = (data.mengajarBayangan || []).filter(
      (item) =>
        String(item.mapel_kode || "").trim() &&
        String(item.guru_kode || "").trim() &&
        String(item.tingkat || "").trim() &&
        String(item.rombel || "").trim(),
    );
    if (context.isAdmin || context.role === "superadmin") return assignments;

    const kodeGuru = context.kodeGuru;
    const coordinatorLevels = new Set(
      (context.coordinatorLevels || [])
    ...
    const coordinatorAssignments = assignments.filter(
      (item) =>
        coordinatorLevels.has(String(item.tingkat || "").trim()) ||
        waliClassSet.has(getHomeAssignmentClassKey(item)),
    );
    const byKey = new Map();
    if (context.isGuru && context.hasCoordinatorAccess) {
      [...ownAssignments, ...coordinatorAssignments].forEach((item) =>
        byKey.set(getHomeAssignmentKey(item), item),
      );
      return [...byKey.values()];
    }
  ...
  function getHomeNilaiInputMode() {
    if (typeof global.resolveNilaiInputModeForCurrentRole === "function") {
      return global.resolveNilaiInputModeForCurrentRole() === "semester"
        ? "semester"
        : "pts";
    }
  ...
  function getHomeSummaryRows(context, data = {}) {
    ...
    const rows = assignments.map((assignment) => {
      const students = getHomeStudentsForAssignment(
        assignment,
        data.siswa || [],
        data.mapelBayangan || [],
      );
      ...
      (data.nilai || []).forEach((item) => {
        ...
        if (
          !current ||
          getHomeNilaiDocTimestamp(item) >= getHomeNilaiDocTimestamp(current)
        ) {
      ...
      const fields = fieldConfigs.map((field) => {
        const filled = students.reduce((count, student) => {
          ...
          const rawValue =
            field.fallback === "nilai"
              ? (nilaiDoc?.uh_1 ?? nilaiDoc?.nilai ?? "")
              : (nilaiDoc?.[field.key] ?? "");
          const value =
            field.key === "rapor" && !String(semesterValue || "").trim()
              ? ""
              : rawValue;
          if (field.key === "rapor") {
            return (
              count +
              (isHomeFilledScoreValue(semesterValue) &&
              isHomeFilledScoreValue(value)
                ? 1
                : 0)
            );
          }
      ...
      const mapel = data.mapelBayangan?.find(
        (item) =>
          String(item.kode_mapel || item.id || "")
            .trim()
            .toUpperCase() ===
  ...
  function buildTeachingRows(assignments = [], mapelList = []) {
    ...
    assignments.forEach((item) => {
      const kode = String(item.mapel_kode || "")
        .trim()
        .toUpperCase();
      if (!kode) return;
      if (!groups.has(kode)) {
      ...
      groups
        .get(kode)
        .kelas.push(
          `${item.tingkat || ""}${String(item.rombel || "").toUpperCase()}`,
        );
    });
  ...
  function buildTeachingComparisonRows(
    originalAssignments = [],
    originalMapel = [],
    realAssignments = [],
    realMapel = [],
  ...
  function renderTeachingTable(
    rows = [],
    emptyMessage = "Belum ada tugas mengajar yang bisa ditampilkan.",
  ) {
    if (!rows.length)
  ...
  function renderTeachingSimpleTable(
    title = "",
    rows = [],
  ...
  function renderSummaryTable(rows = []) {
    if (!rows.length)
      return `<div class="empty-panel">Belum ada pembagian mengajar yang bisa ditampilkan.</div>`;
    const fields =
      rows[0]?.fields || getHomeSummaryFieldConfigs(getHomeNilaiInputMode());
    return `
  ...
  function renderCleanStatCard(id, icon, label, description, iconClass = "") {
    return `
      <article class="stat-card">
        <div class="stat-card-icon ${iconClass}" aria-hidden="true">${icon}</div>
        <div class="stat-card-content">
          <span class="stat-card-label">${escapeHtml(label)}</span>
          <strong class="stat-card-value" id="${escapeHtml(id)}">...</strong>
          <small class="stat-card-description">${escapeHtml(description)}</small>
        </div>
      </article>
    `;
  }

  function renderCleanDashboardIntro(context = {}) {
    const userName = getHomeUserDisplayName(context.user);
    const title = context.isGuru
      ? `Selamat bekerja, ${escapeHtml(userName)}!`
      : `Selamat datang, ${escapeHtml(userName)}!`;
    const subtitle = context.isGuru
      ? "Berikut ringkasan aktivitas mengajar dan data yang perlu dipantau."
      : "Berikut ringkasan aktivitas dan data operasional sekolah.";
    return `
      <section class="dashboard-home">
        <div class="home-header">
          <div class="home-header-left">
            <h2 class="home-greeting">${title}</h2>
            <p class="home-greeting-sub">${escapeHtml(subtitle)}</p>
          </div>
          <div class="home-date-card">
            <span class="home-date-icon" aria-hidden="true">&#128197;</span>
            <span class="home-date-value">${escapeHtml(getFormattedDate())}</span>
          </div>
        </div>
        <div class="stat-cards-grid">
          ${renderCleanStatCard("homeStatSiswa", "S", "Jumlah Siswa", "Siswa aktif", "stat-card-icon--students")}
          ${renderCleanStatCard("homeStatKelas", "K", "Jumlah Kelas", "Kelas aktif", "stat-card-icon--class")}
          ${renderCleanStatCard("homeStatGuru", "G", "Guru & Staf", "Orang", "stat-card-icon--staff")}
          ${renderCleanStatCard("homeStatJadwal", "J", "Jadwal Hari Ini", "Kelas", "stat-card-icon--schedule")}
          ${renderCleanStatCard("homeStatTugas", "T", "Tugas Dinilai", "Tugas", "stat-card-icon--tasks")}
        </div>
      </section>
    `;
  }

  function getHomeStudentName(row = {}) {
    return (
      row.nama ||
      row.nama_siswa ||
      row.nama_lengkap ||
  ...
  function getHomeStudentNis(row = {}) {
    return row.nis || row.nisn || row.nomor_induk || row.id || "-";
  }

  function getHomeStudentStatus(row = {}) {
    const raw = String(row.status || row.status_siswa || row.aktif || "")
      .trim()
      .toLowerCase();
    if (raw === "false" || raw === "nonaktif" || raw === "tidak aktif")
      return "Nonaktif";
    return "Aktif";
  }

  function renderHomeStudentRows(rows = []) {
    const visibleRows = rows.slice(0, 8);
    if (!visibleRows.length) {
      return `
        <tr>
          <td colspan="8">
            <div class="empty-panel">Belum ada data siswa yang bisa ditampilkan.</div>
          </td>
        </tr>
      `;
    }
    return visibleRows
      .map((row) => {
        const parts = getSiswaKelasBayanganParts(row);
        const gender = getJenisKelamin(row);
        const status = getHomeStudentStatus(row);
        return `
          <tr>
            <td><input type="checkbox" aria-label="Pilih siswa ${escapeHtml(getHomeStudentName(row))}"></td>
            <td>${escapeHtml(getHomeStudentNis(row))}</td>
            <td>${escapeHtml(getHomeStudentName(row))}</td>
            <td>${escapeHtml(parts.kelas || row.kelas || "-")}</td>
            <td>${escapeHtml(parts.tingkat || "-")}</td>
            <td>${escapeHtml(gender === "L" ? "Laki-laki" : gender === "P" ? "Perempuan" : "-")}</td>
            <td><span class="status-badge ${status === "Aktif" ? "status-badge-success" : "status-badge-danger"}">${escapeHtml(status)}</span></td>
            <td class="table-actions">
              <button type="button" class="table-action-btn" title="Lihat siswa" aria-label="Lihat siswa" onclick="loadPage('lihat')">V</button>
              <button type="button" class="table-action-btn is-primary" title="Edit siswa" aria-label="Edit siswa" onclick="loadPage('input')">E</button>
              <button type="button" class="table-action-btn is-danger" title="Hapus siswa" aria-label="Hapus siswa" onclick="loadPage('lihat')">D</button>
            </td>
          </tr>
        `;
      })
  ...
  function getHomeStudentFilterOptions(rows = [], type = "kelas") {
    const values = new Set();
    rows.forEach((row) => {
      const parts = getSiswaKelasBayanganParts(row);
      const value = type === "tingkat" ? parts.tingkat : parts.kelas || row.kelas;
      if (value) values.add(String(value));
    });
    return [...values].sort((a, b) => a.localeCompare(b, "id", { numeric: true }));
  }

  function renderHomeStudentPanel() {
    return `
      <section class="module-panel home-student-panel">
        <div class="module-header home-student-header">
          <div class="module-header-left">
            <h3 class="module-title">Data Siswa</h3>
          </div>
          <div class="module-actions home-student-actions">
            <button type="button" class="btn-primary" onclick="loadPage('input')">+ Tambah Siswa</button>
            <button type="button" class="btn-secondary" onclick="loadPage('lihat')">Export</button>
            <button type="button" class="btn-secondary" onclick="window.DashboardHome?.filterHomeStudents?.()">Filter</button>
          </div>
        </div>
        <div class="module-toolbar">
          <div class="module-filters">
            <label class="module-filter">Kelas
              <select id="homeStudentKelasFilter" onchange="window.DashboardHome?.filterHomeStudents?.()">
                <option value="">Semua Kelas</option>
              </select>
            </label>
            <label class="module-filter">Tingkat
              <select id="homeStudentTingkatFilter" onchange="window.DashboardHome?.filterHomeStudents?.()">
                <option value="">Semua Tingkat</option>
              </select>
            </label>
            <label class="module-filter">Status
              <select id="homeStudentStatusFilter" onchange="window.DashboardHome?.filterHomeStudents?.()">
                <option value="">Semua Status</option>
                <option value="Aktif">Aktif</option>
                <option value="Nonaktif">Nonaktif</option>
              </select>
            </label>
            <label class="module-filter module-filter-search">Pencarian
              <input id="homeStudentSearch" type="search" placeholder="Cari nama atau NIS..." oninput="window.DashboardHome?.filterHomeStudents?.()">
            </label>
          </div>
        </div>
        <div class="module-table-wrap home-student-table-wrap">
          <table class="module-table home-student-table">
            <thead>
              <tr>
                <th><input type="checkbox" aria-label="Pilih semua siswa"></th>
                <th>NIS</th>
                <th>Nama Siswa</th>
                <th>Kelas</th>
                <th>Tingkat</th>
            ...
            <tbody id="homeStudentRows">
              <tr><td colspan="8"><div class="empty-panel">Memuat data siswa...</div></td></tr>
            </tbody>
          </table>
          <div class="module-footer">
            <div class="module-footer-left" id="homeStudentCount">Menampilkan 0 data</div>
            <div class="module-footer-right pagination">
              <span>Rows per page:</span>
              <select class="form-control" aria-label="Jumlah baris per halaman"><option>10</option></select>
              <button type="button" class="btn-secondary btn-sm" aria-label="Halaman sebelumnya">&lt;</button>
              <button type="button" class="btn-primary btn-sm">1</button>
              <button type="button" class="btn-secondary btn-sm" aria-label="Halaman berikutnya">&gt;</button>
            </div>
          </div>
        </div>
      </section>
  ...
  function updateHomeStudentPanel(rows = []) {
    lastHomeStudentRows = Array.isArray(rows) ? rows : [];
    const kelasSelect = global.document?.getElementById("homeStudentKelasFilter");
    const tingkatSelect = global.document?.getElementById("homeStudentTingkatFilter");
    if (kelasSelect && kelasSelect.options.length <= 1) {
      kelasSelect.insertAdjacentHTML(
        "beforeend",
        getHomeStudentFilterOptions(lastHomeStudentRows, "kelas")
          .map((value) => `<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`)
          .join(""),
      );
    }
    if (tingkatSelect && tingkatSelect.options.length <= 1) {
      tingkatSelect.insertAdjacentHTML(
        "beforeend",
        getHomeStudentFilterOptions(lastHomeStudentRows, "tingkat")
          .map((value) => `<option value="${escapeHtml(value)}">${escapeHtml(value)}</option>`)
          .join(""),
      );
    }
  ...
  function filterHomeStudents() {
    const doc = global.document;
    const kelas = String(doc?.getElementById("homeStudentKelasFilter")?.value || "");
    const tingkat = String(doc?.getElementById("homeStudentTingkatFilter")?.value || "");
    const status = String(doc?.getElementById("homeStudentStatusFilter")?.value || "");
    const query = String(doc?.getElementById("homeStudentSearch")?.value || "").trim().toLowerCase();
    const filtered = lastHomeStudentRows.filter((row) => {
      const parts = getSiswaKelasBayanganParts(row);
      const rowStatus = getHomeStudentStatus(row);
      const haystack = `${getHomeStudentNis(row)} ${getHomeStudentName(row)} ${parts.kelas || row.kelas || ""}`.toLowerCase();
      return (
        (!kelas || String(parts.kelas || row.kelas || "") === kelas) &&
        (!tingkat || String(parts.tingkat || "") === tingkat) &&
        (!status || rowStatus === status) &&
        (!query || haystack.includes(query))
      );
    });
    global.AppDom?.setHtml?.("homeStudentRows", renderHomeStudentRows(filtered));
    global.AppDom?.setText?.(
      "homeStudentCount",
      `Menampilkan ${Math.min(filtered.length, 8)} dari ${filtered.length || lastHomeStudentRows.length} data`,
    );
  }
  ...
  function renderHomePageShell(context) {
    ...
    return `
      ${renderCleanDashboardIntro(context)}
      ${context.isGuru && !context.isKoordinator ? "" : renderHomeStudentPanel()}
  ...
  function renderGuruHomePage(options = {}) {
    ...
    if (options.pageTitle)
      options.pageTitle.innerText = context.isKoordinator
        ? "Beranda Koordinator"
        : "Beranda Guru";
    if (options.content)
      options.content.innerHTML = renderHomePageShell(context);
    const loadOptions = {
      ...context,
      getCollectionQuery: options.getCollectionQuery,
    };
  ...
  function renderKoordinatorHomePage(options = {}) {
    const context = getHomeRoleContext({
      ...options,
      role: "guru",
      hasCoordinatorAccess: true,
    });
  ...
  async function loadHomeStats(options = {}) {
    ...
    try {
      ...
      const data = global.DashboardHomeData?.loadHomeCollections
        ? await global.DashboardHomeData.loadHomeCollections({
            context,
            getCollectionQuery: options.getCollectionQuery,
          })
        : await (async () => {
            const getQuery = (path) =>
              typeof options.getCollectionQuery === "function"
                ? options.getCollectionQuery(path)
                : documentsApi.collection(path);
            const [
              guru,
              siswa,
              kelas,
      ...
      const nilai = (data.nilai || []).filter((item) =>
        typeof global.isActiveTermDoc === "function"
          ? global.isActiveTermDoc(item)
          : true,
      );
      ...
      setText("homeStatSiswa", `${siswa.length}`);
      setText("homeStatKelas", `${kelas.length}`);
      setText("homeStatGuru", `${guru.length}`);
      setText("homeStatJadwal", `${mengajarBayangan.length || mengajarAsli.length}`);
      setText("homeStatTugas", `${nilai.length}`);
      updateHomeStudentPanel(siswa);
      ...
      const invigilationCard =
        (resolvedContext.isGuru || resolvedContext.isKoordinator) &&
        resolvedContext.kodeGuru
          ? await loadHomeInvigilationCard(
              documentsApi,
              resolvedContext.kodeGuru,
            )
          : null;
      const tugasGuru = resolvedContext.kodeGuru
        ? (await getCachedDocumentRow(
            "guru_tugas_tambahan",
            resolvedContext.kodeGuru,
          )) || {}
        : {};
      const tugasNames = getTugasNames(tugasGuru, tugas);
      const roleAssignments = getHomeVisibleAssignments(inputScopeContext, {
        mengajarBayangan,
        kelas,
      });
      const roleAssignmentsAsli = getHomeVisibleAssignments(inputScopeContext, {
        mengajarBayangan: mengajarAsli,
        kelas,
      });
      const summaryRows = getHomeSummaryRows(inputScopeContext, {
        mengajarBayangan,
        siswa,
        mapelBayangan,
        nilai,
      ...
      const teachingRowsDapodik = buildTeachingRows(
        roleAssignmentsAsli,
        mapelAsli,
      );
      const teachingRowsReal = buildTeachingRows(
        roleAssignments,
        mapelBayangan,
      );
      const siswaByLevel = { 7: 0, 8: 0, 9: 0, lain: 0 };
      ...
      const waliRows = kelas.filter(
        (item) =>
          String(item.kode_guru || "").trim() === resolvedContext.kodeGuru,
      );
      const waliClassSet = new Set(
        waliRows
          .map(
      ...
      global.DashboardShell?.setWaliAccessState?.(
        Boolean(
          resolvedContext.isKoordinator ||
          resolvedContext.isGuruAdmin ||
          waliClassSet.size,
        ),
        [...waliClassSet],
      );
      ...
      if (context.isAdmin || resolvedContext.isGuruAdmin) {
        ...
        setText(
          "homeHeroStat1",
          context.coordinatorLevels.length
            ? context.coordinatorLevels.join(", ")
            : "-",
        );
      ...
      setText(
        "homeInputSummaryTitle",
        summaryRows.length
          ? `${summaryRows.length} pembagian ditampilkan`
          : "Belum ada pembagian yang bisa ditampilkan",
      );
      global.AppDom?.setHtml?.(
        "homeInputSummaryNote",
        `
      ...
      global.AppDom?.setHtml?.(
        "homeInputSummaryTable",
        renderSummaryTable(summaryRows),
      );
      if (
        (resolvedContext.isGuruAdmin || resolvedContext.isKoordinator) &&
        document.getElementById("homeTeachingTable")
      ) {
        setText("homeTeachingTitle", `${teachingRows.length} mapel diajar`);
        global.AppDom?.setHtml?.(
          "homeTeachingTable",
          renderTeachingDualTable(teachingRowsDapodik, teachingRowsReal),
        );
      }
      ...
      if (context.isAdmin) {
        ...
        setText(
          "homeAdminCleanText",
          `Ringkasan utama menampilkan ${summaryRows.length} kelas-mapel aktif dan ${onlineUsers.length} user online.`,
        );
      } else if (resolvedContext.isGuruAdmin) {
        setText("homeRoleCardTitle1", `${onlineUsers.length} pengguna aktif`);
        setText(
          "homeRoleCardTitle2",
          `${guru.length} guru, ${kelas.length} kelas, ${mapelBayangan.length} mapel`,
        );
        ...
        setText(
          "homeRoleCardTitle1",
          `${context.coordinatorLevels.length ? context.coordinatorLevels.join(", ") : "-"} aktif`,
        );
        setText("homeRoleCardTitle2", `${waliClassSet.size} kelas wali`);
        setText("homeRoleCardTitle3", "Pintasan kerja utama");
        setText(
          "homeInvigilationTitle",
          invigilationCard?.exam_type
            ? String(invigilationCard.exam_type || "").toUpperCase()
            : "Belum ada kartu pengawas",
        );
        setText(
        ...
        setText(
          "homeHeroTaskTitle",
          tugasNames.length
            ? `${tugasNames.length} tugas tambahan`
        ...
        global.AppDom?.setHtml?.(
          "homeInvigilationContent",
          renderHomeInvigilationCard(invigilationCard),
        );
      } else {
        const teachingRowsGuru = buildTeachingComparisonRows(
          getHomeVisibleAssignments(context, {
            mengajarBayangan: mengajarAsli,
            kelas,
          }),
          mapelAsli,
        ...
        const waliStudents = waliClass
          ? siswa
              ...
              .filter((item) => item.kelasBayanganParts.kelas === waliClass)
          : [];
        const genderCounts = waliStudents.reduce(
          (result, item) => {
            const gender = getJenisKelamin(item);
        ...
        setText(
          "homeRoleCardTitle1",
          teachingRowsGuru.length
            ? `${teachingRowsGuru.length} mapel diajar`
            : "Belum ada tugas mengajar",
        );
        setText(
          "homeRoleCardTitle2",
          waliClass ? `Kelas ${waliClass}` : "Belum menjadi wali kelas",
        );
        setText(
          "homeInvigilationTitle",
          invigilationCard?.exam_type
        ...
        global.AppDom?.setHtml?.(
          "homeRoleCardList1",
          renderTeachingDualTable(
            teachingRowsGuruDapodik,
            teachingRowsGuruReal,
          ),
        );
        global.AppDom?.setHtml?.(
          "homeRoleCardList2",
      ...
      setText(
        "homeInputSummaryTitle",
        "Rangkuman input belum berhasil dimuat.",
      );
      setHomeInvigilationHeroVisibility(false);
      global.AppDom?.setHtml?.(
        "homeInputSummaryTable",
        `<div class="empty-panel">Data rangkuman input belum berhasil dimuat.</div>`,
      );
    } finally {
  ...
  async function refreshHome() {
    ...
    homeRefreshInFlight = loadHomeStats({ ...options, force: true }).finally(
      () => {
