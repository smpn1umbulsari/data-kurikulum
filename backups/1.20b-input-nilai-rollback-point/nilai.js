    ...
    <div class="card">
      ...
      <div class="nilai-control-panel">
        ...
        <div class="nilai-control-actions">
          <details class="nilai-download-menu" id="nilaiDownloadMenu">
          ...
          <button type="button" id="nilaiSaveButton" class="btn-primary" onclick="saveNilaiAssignment()">Simpan</button>
      ...
      ${canUseNilaiOfflineDraft() ? `<div id="nilaiOfflineDraftInfo" class="nilai-offline-note" hidden></div>` : ""}
...
function renderNilaiInputModeUi() {
  ...
  const saveButton = document.getElementById("nilaiSaveButton");
  ...
  if (saveButton) {
    ...
    saveButton.title = shouldSaveNilaiAsOfflineDraft()
      ? "Nilai akan disimpan sebagai draft lokal lalu dikirim otomatis saat internet tersedia."
      : "Nilai disimpan lebih dulu sebagai draft aman, lalu langsung dikirim ke server.";
  }
}
...
function renderNilaiOfflineDraftInfo(assignment) {
  ...
  const listItems = allDrafts.slice(0, 5).map(item => {
    const draftAssignment = item.draft?.assignment?.mapel_kode
      ? item.draft.assignment
      : (() => {
          const [, tingkat = "", rombel = "", mapelKode = "", guruKode = ""] = String(item.assignmentKey || "").split("|");
          return { tingkat, rombel, mapel_kode: mapelKode, guru_kode: guruKode };
        })();
    ...
    return `
      <li class="${item.assignmentKey === activeKey ? "is-active" : ""}">
        <strong>${escapeNilaiHtml(label)}</strong>
        <span>${Number(item.rows || 0)} baris · ${escapeNilaiHtml(savedAt)}${item.assignmentKey === activeKey ? " · sedang dibuka" : ""}</span>
      </li>
    `;
  }).join("");
  panel.hidden = false;
  panel.innerHTML = `
    <div class="nilai-draft-summary-head">
      <div>
        ...
        <span>${totalRows} baris nilai masih aman di perangkat ini dan akan sinkron otomatis saat online.</span>
      </div>
    </div>
    <ul class="nilai-draft-summary-list">${listItems}</ul>
  `;
}

function closeNilaiDownloadMenu() {
  const menu = document.getElementById("nilaiDownloadMenu");
...
function getNilaiRowUiValuesFromPayload(payload = {}) {
  return getNilaiUiValues({
    nilai: payload?.uh_1 ?? "",
    uh_1: payload?.uh_1 ?? "",
    uh_2: payload?.uh_2 ?? "",
    uh_3: payload?.uh_3 ?? "",
...
function isNilaiRowChanged(row, assignment) {
  const previousValues = getNilaiUiValues(getNilaiForStudent(assignment, row?.siswa?.nipd));
  const nextValues = getNilaiRowUiValuesFromPayload(row?.payload || {});
  return ["uh1", "uh2", "uh3", "uh4", "uh5", "pts", "semester", "rapor"]
    .some(key => String(previousValues[key] ?? "") !== String(nextValues[key] ?? ""));
}
...
async function saveNilaiAssignmentOfflineDraft(options = {}) {
  ...
  try {
    const rows = buildNilaiRowsFromCurrentInputs(assignment, user).filter(row => isNilaiRowChanged(row, assignment));
    if (!rows.length) return { saved: false, reason: "no-change" };
    ...
    renderNilaiOfflineDraftInfo(assignment);
    if (!silent) {
      Swal.fire("Draft tersimpan", "Nilai disimpan di perangkat ini. Sinkron akan berjalan otomatis saat internet tersedia.", "success");
    }
    return { saved: true, rows, assignment, assignmentKey, user };
  } catch (error) {
    console.error(error);
...
async function syncAllNilaiOfflineDrafts(options = {}) {
  ...
  if (!window.GuruOffline?.isOnline?.()) return { synced: 0, failed: 0, skipped: true };
  const user = getCurrentNilaiUser();
  ...
  try {
    for (const item of drafts) {
      ...
      const draftAssignment = draft?.assignment;
    ...
    window.GuruOffline?.renderStatus?.();
    if (!automatic && failed) {
      Swal.fire("Sinkron draft sebagian gagal", `${synced} baris terkirim, ${failed} draft belum berhasil.`, "warning");
    }
    return { synced, failed };
  } catch (error) {
    console.error(error);
    if (!automatic) Swal.fire("Gagal sinkron", error?.message || "Draft belum berhasil disinkronkan.", "error");
...
function initNilaiAutoSync() {
  ...
  window.addEventListener?.("online", () => {
    window.setTimeout(() => autoSyncNilaiOfflineDrafts(), 700);
  });
  document.addEventListener?.("visibilitychange", () => {
    if (!document.hidden) window.setTimeout(() => autoSyncNilaiOfflineDrafts(), 700);
  });
  window.setTimeout(() => autoSyncNilaiOfflineDrafts(), 900);
}
...
async function saveNilaiAssignment() {
  ...
  if (canSaveNilaiAsOfflineDraft()) {
    const draftResult = await saveNilaiAssignmentOfflineDraft({ silent: true });
    if (!draftResult?.saved) return;
    if (shouldSaveNilaiAsOfflineDraft()) return;
    try {
      setNilaiSavingState(true, "Mengirim nilai...");
