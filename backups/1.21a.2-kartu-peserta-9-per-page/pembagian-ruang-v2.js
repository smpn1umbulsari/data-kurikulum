...
function renderAsesmenKartuPesertaCard(entry, meta = {}) {
  const kepalaInfo = meta.kepalaInfo || {};
  ...
  const ttdImage = kepalaInfo.ttd
    ? `<img class="kartu-peserta-ttd" src="${escapeAsesmenHtml(kepalaInfo.ttd)}" alt="TTD Kepala Sekolah">`
    : `<div class="kartu-peserta-ttd kartu-peserta-ttd--empty"></div>`;
...
function getKartuPesertaPrintHtml(level) {
  const levelKey = String(level);
  const entries = getAsesmenKartuPesertaEntries(levelKey);
  const chunks = [];
  for (let index = 0; index < entries.length; index += 9) {
    chunks.push(entries.slice(index, index + 9));
  }
  if (!chunks.length) chunks.push([]);
  ...
  const pages = chunks.map(chunk => renderAsesmenKartuPesertaPage(chunk, meta)).join("");
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>Kartu Peserta Kelas ${escapeAsesmenHtml(levelKey)}</title>
      <style>
        * { box-sizing: border-box; }
        html, body {
          margin: 0;
...
async function promptExportKartuPesertaPDF() {
  ...
  const html = getKartuPesertaPrintHtml(level);
